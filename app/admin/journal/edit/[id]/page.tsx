    ...
    const fetchArticle = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          let parsedContent = "";
          try {
            const arr = typeof article.content === 'string' ? JSON.parse(article.content) : article.content;
            if (Array.isArray(arr)) {
              parsedContent = arr.map((p: string) => `<p>${p}</p>`).join("");
            } else {
        ...
        toast.error("Network error: Failed to load article");
      } finally {
        setFetching(false);
      }
    };
