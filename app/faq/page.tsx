...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-gray-100 last:border-b-0">
      <button onClick={() => setOpen(!open)} className="w-full flex justify-between items-start py-6 text-left group">
        <h3 className="text-lg font-serif text-[#111810] pr-8 group-hover:text-[#1a4a38] transition-colors">{q}</h3>
        <ChevronDown size={20} className={`text-[#e2a325] flex-shrink-0 mt-1 transition-transform duration-300 ${open ? "rotate-180" : ""}`} />
      </button>
      {open && <div className="pb-6 text-gray-600 font-light leading-relaxed text-sm pr-12">{a}</div>}
    </div>
  );
}
    ...
    <div className="bg-[#1a1c17] text-[#f4f2eb] min-h-screen overflow-hidden font-sans">
      ...
      <section className="relative pt-40 pb-32 px-6 lg:px-24 bg-[#fdfaf6] overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-serif text-[#111810] leading-tight mb-6">Frequently Asked Questions</h1>
          <p className="text-lg lg:text-xl text-gray-600 max-w-2xl font-light leading-relaxed">Everything you need to know about our cold-pressed oils, ordering, and delivery.</p>
        </div>
      </section>

      {/* FAQ LIST */}
      <section className="bg-white py-24 px-6 lg:px-24">
        <div className="max-w-3xl mx-auto bg-white rounded-[2rem] border border-gray-100 p-8 md:p-12 shadow-xl shadow-black/5">
      ...
      <section className="bg-[#fdfaf6] py-24 px-6 lg:px-24 border-t border-gray-100">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-serif text-[#111810] mb-6">Still Have Questions?</h2>
          <p className="text-gray-600 font-light leading-relaxed mb-10 max-w-lg mx-auto">Can&apos;t find the answer you&apos;re looking for? Reach out and we&apos;ll get back to you within 24 hours.</p>
          <Link href="/contact"><PrimaryButton className="px-10 py-4 text-xs tracking-widest shadow-lg shadow-black/5"><MessageCircle size={14} className="mr-2" /> Contact Us</PrimaryButton></Link>
        </div>
      </section>
